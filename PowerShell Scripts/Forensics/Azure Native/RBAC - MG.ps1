...
Write-Output ("Removing " + $memCapDisk.Name + " from Forensics VM if attached")
if ($forensicsVM.StorageProfile.DataDisks.Name -contains $memCapDisk.Name)
{
	Remove-AzVMDataDisk -VM $forensicsVM -Name $memCapDisk.Name
...
New-AzStorageAccount -ResourceGroupName $attackedVM.ResourceGroupName -Name $saName -SkuName Standard_LRS -Location $attackedVM.Location -Kind StorageV2 -AccessTier Hot
...
$storageAccount = Get-AzStorageAccount -ResourceGroupName $attackedVM.ResourceGroupName -Name $saName
...
Remove-AzStorageAccount -ResourceGroupName $storageAccount.ResourceGroupName -Name $storageAccount.StorageAccountName -Force -Confirm:$false
...
Write-Output "Generating SAS token for managed disk to extract VHD"
$forensicSAS = Grant-AzDiskAccess -ResourceGroupName $attackedVM.ResourceGroupName -DiskName $newDiskName -DurationInSecond $sasExpiryDuration -Access Read
...
#Attach memcap disk to Forensic VM
if ($oldCapture = Get-AzDisk | Where-Object { $_.Name -like "*memcap*"})
{
	foreach ($capture in $oldCapture) {
		Remove-AzVMDataDisk -VM $forensicsVM -Name $capture.Name
		Update-AzVM -ResourceGroupName $forensicsVM.ResourceGroupName -VM $forensicsVM
		$forensicsVM = Get-AzVM -ResourceGroupName $forensicsVM.ResourceGroupName -Name $forensicsVM.Name
		Write-Host ($capture.Name + " has been detached from $forensicsVMName. Please delete if no longer needed") -ForegroundColor yellow -BackgroundColor Black
		$lun = $null
		$numberOfDataDisks = $forensicsVM.StorageProfile.DataDisks.Count
		if ($numberOfDataDisks -eq 0)
		{
