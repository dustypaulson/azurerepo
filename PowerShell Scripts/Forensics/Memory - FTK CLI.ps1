...
if (!(Get-Process -Name ftkimager -ErrorAction SilentlyContinue) -and !(Test-Path ($memCapDriveLetter + $env:computername)))
{
	...
	@"
    $drive
	cd "$($memCapDrive.Name):\ftkimager\"
    mkdir "$($memCapDriveLetter)$($env:computername)"
	ftkimager.exe \\.\PhysicalDrive0 $folder --e01 --frag 2G --compress 9 --verify
	exit
